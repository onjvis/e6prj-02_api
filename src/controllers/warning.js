...
module.exports.getWarning = async function(req, res, next) {
    try {
        if (!ObjectId.isValid(req.params['warningId']) || !await Warning.exists({ _id: req.params['warningId'] })) {
...
module.exports.createWarning = async function(req, res, next) {
    if (!req.body.binId || !req.body.type || !req.body.timestamp) {
        ...
        try {
            const warning = new Warning({ binId: req.body.binId, type: req.body.type, timestamp: req.body.timestamp });
            if (req.body.message) {
